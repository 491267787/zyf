...
//	    close(sockfd);
	    ...
	    //读取服务器发送的数据
	    while(1){
	    struct json_object *jiebao_json,*jiebao_mode,*jiebao_apname,*jiebao_appassword,*jiebao_staname,*jiebao_stapassword,*jiebao_pppoename,*jiebao_pppoepassword,*jiebao_usrname,*jiebao_usrpassword=NULL;
	    memset(buff,0,sizeof(buff));
...
//	    buff[n]='\0';
	    ...
	    printf("mode:%d\t",mode);    //1为AP模式，2为STA模式
